...
export async function createStatus(
  ...
  const existing = await repo.getBySlug(ownerId, slug);
  ...
  const statuses = await repo.list(ownerId);
...
export async function listStatuses(
  ...
  return ok(await repo.list(ownerId));
}
...
export async function deleteStatus(
  ...
  const status = await repo.getById(id, ownerId);
  if (!status) return err('NOT_FOUND', 'Status not found'); // miss/not-owned → 404 (no disclosure)
  if (status.isDefault) return err('CONFLICT', 'Cannot delete the default status');

  const inUse = await repo.countTasks(id, ownerId);
