...
// In-memory StatusRepository built to the same port contract: owner-scoping, position ordering, and
...
export class InMemoryStatusRepository implements StatusRepository {
  ...
  taskCounter: (statusId: string, ownerId: string) => number = () => 0;

  async list(ownerId: string) {
    return this.rows.filter((s) => s.ownerId === ownerId).sort((a, b) => a.position - b.position);
  }

  async getById(id: string, ownerId: string) {
    return this.rows.find((s) => s.id === id && s.ownerId === ownerId) ?? null;
  }

  async getBySlug(ownerId: string, slug: string) {
    return this.rows.find((s) => s.ownerId === ownerId && s.slug === slug) ?? null;
  }

  async getDefault(ownerId: string) {
    const owned = await this.list(ownerId);
    return owned.find((s) => s.isDefault) ?? owned[0] ?? null; // isDefault, else lowest position
  }

  async countTasks(id: string, ownerId: string) {
    return this.taskCounter(id, ownerId);
  }
  ...
  async delete(id: string, ownerId: string) {
    const idx = this.rows.findIndex((s) => s.id === id && s.ownerId === ownerId);
